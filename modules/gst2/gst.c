...
/*	st->source = gst_element_factory_make("playbin", "source");
	...
	*/
	//alsasrc name=alsasrc0 device=hw:0,0 do-timestamp=true latency-time=20000 buffer-time=800000
	//name=alsacaps0 caps=audio/x-raw,rate=${audio_rate},channels=2,depth=16
	st->source = gst_element_factory_make("alsasrc", "source");
	if (!st->source) {
			warning("gst: failed to create alsasrc source element\n");
			return ENOMEM;
	}
	g_object_set(G_OBJECT(st->source), "device", "hw:0,0", NULL);
	g_object_set(G_OBJECT(st->source), "do-timestamp", TRUE, NULL);
	g_object_set(G_OBJECT(st->source), "latency-time", 20000UL, NULL);
	g_object_set(G_OBJECT(st->source), "buffer-time", 800000UL, NULL);

	st->capsfilt2 = gst_element_factory_make("capsfilter", NULL);
	if (!st->capsfilt2) {
		warning("gst: failed to create capsfilter element\n");
		return ENOMEM;
	}
	g_object_set(G_OBJECT(st->capsfilt2), "caps", "audio/x-raw,rate=48000,channels=2,depth=16 ", NULL);

	st->auconv = gst_element_factory_make("audioconvert", NULL);
	if (!st->auconv) {
			warning("gst: failed to create  audioconvert element\n");
			return ENOMEM;
	}
	...
	//g_object_set(G_OBJECT(st->source), "audio-sink", st->bin, NULL);
	gst_element_link_many(st->source, st->capsfilt2, st->auconv, st->bin, NULL);
