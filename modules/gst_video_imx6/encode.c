					   ...
					   gpointer user_data)
{
	...
	if (sample) {
		...
		if (ts == GST_CLOCK_TIME_NONE) {
			warning("gst_video_imx6: timestamp is unknown\n");
			rtp_ts = 0;
		}
		else {
					   ...
					   struct videnc_state *st)
{
	...
	if ((GST_MESSAGE_TYPE (msg)) == GST_MESSAGE_ERROR) {
		...
		warning("gst_video_imx6: Error: %d(%m) message=%s\n",
			err->code, err->code, err->message);
		warning("gst_video_imx6: Debug: %s\n", dbg_info);
		g_error_free (err);
		g_free (dbg_info);
...
//	size->w, size->h, st->encoder.fps,
	//warning("gst_video_imx6: launch : %d x %d\n",size->w, size->h);
	...
	snprintf(pipeline, sizeof(pipeline),
	 ...
	 "imxvpuenc_h264 bitrate=%d name=enc0 gop-size=16 ! "
	 "video/x-h264,profile=high ! "
     //"rtph264pay name=pay0 pt=96 ! "
	 "appsink name=sink emit-signals=TRUE drop=TRUE",
	 size->w, size->h, st->encoder.fps,
	 st->encoder.bitrate / 1000 /* bit/s */);

	/* Initialize pipeline. */
	st->streamer.pipeline = gst_parse_launch(pipeline, &gerror);

	if (gerror) {
		warning("gst_video_imx6: launch error: %d: %s: %s\n",
			  ...
			  void *arg)
{
	...
	if (0 == pl_strcasecmp(name, "packetization-mode")) {
		...
		if (st->h264.packetization_mode != 0) {
			warning("gst_video_imx6: illegal packetization-mode %u\n",
		...
		struct pl prof = *val;
		if (prof.l != 6) {
			warning("gst_video_imx6: invalid profile-level-id (%r)\n",
				val);
			return;
		}
			  ...
			  videnc_packet_h *pkth, void *arg)
{
	...
	if (!st) {
		...
		if (err) {
			warning("gst_video_imx6: resource allocation failed\n");
			return err;
		}
		st = *stp;
	...
	info("gst_video_imx6: video encoder %s: %d fps, %d bit/s, pktsize=%u\n",
...
static int pipeline_push(struct videnc_state *st, const struct vidframe *frame)
{
	...
	if (ret != GST_FLOW_OK) {
		warning("gst_video_imx6: pushing buffer failed\n");
		err = EPROTO;
		goto out;
	}
	...
	if (ret != GST_FLOW_OK) {
		warning("gst_video_imx6: pushing EOS failed\n");
		err = EPROTO;
		goto out;
	}
#endif
		     ...
		     const struct vidframe *frame)
{
	...
	if (!st->streamer.valid ||
		...
		if (err) {
			warning("gst_video_imx6: pipeline initialization failed\n");
			return err;
		}
	...
	if (update) {
		debug("gst_video_imx6: gstreamer picture update"
